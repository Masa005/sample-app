...
class HomeView(LoginRequiredMixin,generic.FormView):
    ...
    def get_context_data(self, **kwargs):
        ...
        #ログインユーザーの投稿内容一覧を取得
        user_post_list = PostContent.objects.filter(user__username = self.
                                                    request.user.username).select_related().all().order_by(
                                                        'date_joined').reverse()
        #投稿内容一覧を10個ずつ表示
...
def post_load_api(request):
    ...
    """
    #ログインユーザーの投稿内容一覧を取得
    user_post_list = PostContent.objects.filter(user__username =
                                                request.user.username).select_related().all().order_by(
                                                    'date_joined').reverse()
    #投稿内容一覧を10個ずつ表示
    paginator = Paginator(user_post_list,10)
